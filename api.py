...
def melhor_opcao(montante, valor_aluguel, aumento_aluguel, valorizacao_imovel, juros_aplicacao):
    ...
    if opcao1 > opcao2:
        ...
        for i in range(20*12):
            resultado = (
                valores_aplicacao[i] - acumulado_aluguel[i]) - valores_da_casa[i]
        ...
        mes_virada = -1
        for i in range(20*12):
            resultado = valores_da_casa[i] - \
                (valores_aplicacao[i] - acumulado_aluguel[i])
            lista_resultado.append(resultado)
            if resultado > 0:
                if mes_virada == -1:
                    mes_virada = i + 1
...
def home(montante: float,
    ...
    [melhor, valor_acumulado, numero_mes] = melhor_opcao(
        montante, valor_aluguel, aumento_aluguel, valorizacao_imovel, juros_aplicacao)

    # Obtém a data atual.
    hoje = datetime.datetime.now()

    # Adiciona o número de meses à data atual.
    data_final = hoje + relativedelta(months=numero_mes)

    # Obtém o mês
    mes_str = mes_from_int(data_final.month)
    ano = data_final.year
